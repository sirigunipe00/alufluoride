...
typedef GateEntriesCubit = InfiniteListCubit<GateEntryForm, Pair<int?, String?>,
    Pair<int?, String?>>;
typedef GateEntriesCubitState = InfiniteListState<GateEntryForm>;

typedef GateEntryLinesCubit
    = NetworkRequestCubit<List<GateEntryLinesForm>, String>;
typedef GateEntryLinesCubitState
...
typedef VehicleRequestList
    = NetworkRequestCubit<List<VehcileRequestForm>, None>;
...
typedef ProcessImgCubit = NetworkRequestCubit<ImpProcessInp, ImpProcessInp>;
typedef ProcessImgState = NetworkRequestState<ImpProcessInp>;
...
class GateEntryBlocProvider {
  ...
  GateEntriesCubit fetchGateEntries() => GateEntriesCubit(
        requestInitial: (params, state) =>
            repo.fetchEntries(0, params!.first, params.second),
  ...
  MaterialNameList materialNameList() =>
      MaterialNameList(onRequest: (_, state) => repo.materialName());

  SupplierNameList supplierNameList() =>
      SupplierNameList(onRequest: (_, state) => repo.supplierName());

  VehicleRequestList vehicleRequestCubit() =>
      VehicleRequestList(onRequest: (_, state) => repo.fetchVechicleEntries());

  VehicleList vehicleListCubit() =>
      VehicleList(onRequest: (_, state) => repo.fetchVehicleList());

  PurchaseOrderList purchaseOrderListCubit() =>
      PurchaseOrderList(onRequest: (_, state) => repo.fetchPurchaseOrderList());
  ...
  ReceiverNameList receiverNameList() => ReceiverNameList(
        onRequest: (_, state) => repo.receiverName(),
      );

  // DeleteLines removeGaylord() => DeleteLines(
  // onRequest:(params, state) =>repo.deleteLines(params!.first, params.second));

  ProcessImgCubit processImg() => ProcessImgCubit(
        onRequest: (params, _) => repo.uploadImage(params!),
      );
}
